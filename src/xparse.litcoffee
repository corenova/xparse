...
Please refer to the main package [README](../README) for examples.
    ...
    wrap = (func, x, y=null) ->
      f = exec.bind func, x, y
      f.bindings = arguments
      f.inspect = -> f.bindings
      switch
        when func?     then debug "wrap:", x, func.name, y
        when x? and y? then debug "wrap: #{x}(#{y})"
        when x?        then debug "wrap: #{x}"
      return P.unit f

Here we introduce an *extension* to the parser to simplify
expression of recursive precedence handling.
...
Here we introduce an *extension* to the parser to simplify
expression of elements enclosed by a common pattern.
...
A single-quoted string is matched as-is.
...
*evaluation* of the expression.
    ...
    exports = module.exports = parse['default'] = parse
